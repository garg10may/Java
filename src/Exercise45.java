...
public class Exercise45 {
	...
	public static ArrayList<String> find(ArrayList<String> chain) {
		...
		if ( map.get(last_character)!= null) { //Java null pointer exception is just frustrating
		
		options.addAll(map.get(last_character));
...
*/
//still not able to debug why the HashSet becomes empty for some character for no reason. 
...
public class Exercise45 {
	...
	public static void makeHashMap() {
		...
		for (String word : pokemonArray) {

			char first_character = word.charAt(0);

			map.get(first_character).add(word);
		}
	...
	public static ArrayList<String> find(ArrayList<String> chain) {
		...
		//syntactically correct and innocous looking this is very dangerous. 
		//Hmm after hours, here options is referencing the map set therefore any operation 
		//on it will bring changes to map as well. Always use new keyword
		
		if (map.get(last_character)!= null) {
		...
		try {
			...
			options.removeAll(chain); 
			//this will remove values from original map it not created using new. 
